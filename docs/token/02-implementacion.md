Chaincode básico para comprobar la interración dentro de la red.

> Es recomendable mantener la estructura de carpetas de Hyperledger `fabric-samples` y sustituir este chaincode por el que trae Hyperledger por defecto. Si creamos una carpeta nueva para este chaincode es muy probable que los comandos de despliegue y uso no funcionen correctamente.


## Código implementado del Chaincode
```go
package main
...
> Es importante ejecutar el comando `go mod tidy` antes de realizar pruebas sobre este token.